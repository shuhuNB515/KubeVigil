...
KubeVigil 是一个基于 **eBPF** 的 Kubernetes 运行时安全工具。它直接在 Linux 内核态监控系统调用，实时捕获容器内的异常行为（反弹 Shell、挖矿程序、敏感文件访问、C2 通信等），并自动执行隔离、终止等响应策略。

### 为什么需要 KubeVigil？

| 传统静态扫描 | KubeVigil 运行时防护 |
|---|---|
| 检查镜像中已知漏洞 | 捕获运行时的未知攻击 |
| 检查 YAML 配置规范 | 监控实际系统调用行为 |
| 无法发现 0day 利用 | 内核态视角，无死角 |
| 无法发现无文件攻击 | 捕获内存马、反弹 Shell |
| 事后审计 | 实时检测 + 自动响应 |
